...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ✅ SAVE ALL FEATURES HERE (BEFORE REFINEMENT)
    if SAVE_LOCAL:
        all_features_path = os.path.join(
            os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
            "data", "final", "final_features.csv"
        )
        os.makedirs(os.path.dirname(all_features_path), exist_ok=True)
        df.to_csv(all_features_path, index=False)
        print(f"💾 ALL FEATURES saved → {all_features_path}")
