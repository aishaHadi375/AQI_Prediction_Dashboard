# """
# FastAPI Backend for Karachi AQI Prediction System
...
#     model_used: str
#     generated_at: str
#     daily_summary: List[Dict[str, float]]
...
#         if not model_path:
#             raise FileNotFoundError(f"Model not found: {model_name}.pkl")
...
#         if not api_key:
#             # Generate demo data
#             FEATURE_STORE_DATA = generate_demo_data()
#             return
...
#         FEATURE_STORE_DATA = df.sort_values("datetime").reset_index(drop=True)
        
#     except Exception as e:
#         print(f"Feature Store error: {e}. Using demo data.")
#         FEATURE_STORE_DATA = generate_demo_data()
...
#     base_aqi = 120
#     hourly_pattern = np.sin(np.arange(720) * 2 * np.pi / 24) * 25
#     weekly_pattern = np.sin(np.arange(720) * 2 * np.pi / 168) * 15
#     noise = np.random.normal(0, 12, 720)
...
# def prepare_features(df, metadata=None):
...
#         "version": "1.0.0",
#         "endpoints": {
#             "/current": "Get current AQI",
#             "/predict": "Get 3-day predictions",
...
#         "data_loaded": FEATURE_STORE_DATA is not None,
...
#     if FEATURE_STORE_DATA is None:
#         raise HTTPException(status_code=503, detail="Data not available")
    
#     latest = FEATURE_STORE_DATA.iloc[-1]
...
#     """Generate AQI predictions for specified hours"""
#     if MODEL is None:
...
#         last_date = FEATURE_STORE_DATA['datetime'].max()
#         future_dates = [last_date + timedelta(hours=i) for i in range(1, request.hours + 1)]
        
#         base_features = prepare_features(FEATURE_STORE_DATA, METADATA)
...
#                 if col in row.index:
#                     row[col] = row[col] * (1 + np.random.normal(0, 0.04))
...
# @app.get("/historical", response_model=HistoricalDataResponse)
# async def get_historical_data(hours: int = 168):
#     """Get historical AQI data"""
#     if FEATURE_STORE_DATA is None:
#         raise HTTPException(status_code=503, detail="Data not available")
    
#     data = FEATURE_STORE_DATA.tail(hours)
...
#     """Refresh data from Feature Store"""
#     try:
#         fetch_feature_store_data()
#         return {
#             "status": "success",
#             "message": "Data refreshed successfully",
#             "records": len(FEATURE_STORE_DATA) if FEATURE_STORE_DATA is not None else 0,
#             "timestamp": datetime.now().isoformat()
#         }
#     except Exception as e:
